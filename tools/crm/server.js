...
app.use('/api', (req, res, next) => {
  ...
  if (req.path === '/scouts/apply' || req.path.startsWith('/scouts/dashboard/') || req.path.startsWith('/scouts/by-code/') || req.path.startsWith('/internal/') || req.path === '/sms/webhook/inbound' || req.path === '/voice/webhook' || req.path === '/kingdom-reach/health' || req.path === '/admin/seed-outreach' || req.path === '/outreach/mark-replied' || req.path === '/kingdom-reach/churches/import' || req.path === '/kingdom-reach/churches/mark-opened' || req.path === '/kingdom-reach/churches/mark-replied-church' || req.path === '/kingdom-reach/churches/mark-bounced' || req.path === '/kingdom-reach/enrich' || req.path === '/kingdom-reach/churches' || req.path.startsWith('/kingdom-reach/track/') || req.path === '/kingdom-reach/campaign/send' || req.path === '/kingdom-reach/campaign/preview' || req.path === '/kingdom-reach/workspace-settings' || req.path.startsWith('/kingdom-reach/workspace-settings/') || req.path === '/kingdom-reach/stats/last-7-days' || req.path === '/kingdom-reach/variants/stats') return next();
  // Token-seeded PATCH for individual church record updates from CLI scripts
  // (fail-closed: token must be SET, no KingdomSeed2026 fallback)
  const SEED_TOKEN = process.env.SEED_TOKEN;
  if (SEED_TOKEN && req.method === 'PATCH' && req.path.startsWith('/kingdom-reach/churches/') && req.body?.token === SEED_TOKEN) return next();
  const session = validateSession(getCookie(req, 'crm_session'));
